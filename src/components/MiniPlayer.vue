...
<template>
  <aside v-if="player.currentTrack" class="mini-player">
    <button type="button" class="mini-player-open" :aria-label="t('player.openNowPlaying', { title: player.currentTrack.title })" @click="router.push('/now-playing')">
    ...
    <button type="button" class="mini-toggle-wrap" :aria-label="player.isPlaying ? t('player.pause') : t('player.play')" @click="player.togglePlayback()">
      ...
      <span class="mini-toggle"><Pause v-if="player.isPlaying" :size="15" fill="currentColor" /><Play v-else :size="15" fill="currentColor" /></span>
    </button>
  </aside>
</template>
