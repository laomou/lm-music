...
<template>
  <section class="page downloads-page">
    ...
    <div class="storage-card"><div><small>浏览器缓存</small><strong>{{ formattedBytes }} 音乐</strong><span>站点已使用 {{ usage }}</span></div><button v-if="downloads.tracks.length" class="danger-button" @click="downloads.clearAll()"><Trash2 :size="16" /> 清除全部</button></div>
