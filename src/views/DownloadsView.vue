...
<template>
  <section class="page downloads-page">
    ...
    <div class="storage-card"><div><small>{{ t('downloads.cache') }}</small><strong>{{ t('downloads.music', { size: formatBytes(downloads.totalBytes) }) }}</strong><span>{{ t('downloads.siteUsage', { size: formatBytes(downloads.storageUsage) }) }}</span></div><button type="button" v-if="downloads.tracks.length" class="danger-button" @click="clearAllDownloads"><Trash2 :size="16" /> {{ t('downloads.clearAll') }}</button></div>
