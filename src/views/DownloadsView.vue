...
const storageText = computed(() => downloads.storageQuota > 0
  ? t('downloads.siteUsageQuota', { usage: formatBytes(downloads.storageUsage), quota: formatBytes(downloads.storageQuota) })
  : t('downloads.siteUsage', { size: formatBytes(downloads.storageUsage) }))
onMounted(() => {
  downloads.refresh()
...
function clearAllDownloads() {
  if (window.confirm(t('downloads.confirmClearAll'))) downloads.clearAll()
}

function removeDownload(track: typeof downloads.tracks[number]) {
  if (window.confirm(t('downloads.confirmRemove', { title: track.title }))) downloads.remove(track)
}
</script>
...
<template>
  <section class="page downloads-page">
    ...
    <div class="storage-card"><div><small>{{ t('downloads.cache') }}</small><strong>{{ t('downloads.music', { size: formatBytes(downloads.totalBytes) }) }}</strong><span>{{ storageText }}</span></div><button type="button" v-if="downloads.tracks.length" class="danger-button" @click="clearAllDownloads"><Trash2 :size="16" /> {{ t('downloads.clearAll') }}</button></div>
    ...
    <section class="downloaded-tracks"><h2>{{ t('downloads.songs') }}</h2><p v-if="!downloads.tracks.length" class="empty-copy">{{ t('downloads.empty') }}</p><div v-else class="track-list"><div v-for="track in downloads.tracks" :key="track.id" class="download-row"><span class="offline-badge"><Check :size="14" /></span><span class="track-copy"><strong :title="track.title">{{ track.title }}</strong><small :title="track.artist">{{ track.artist }}</small></span><button type="button" class="download-icon" :aria-label="t('downloads.removeTrack', { title: track.title })" @click="removeDownload(track)"><X :size="17" /></button></div></div></section>
  </section>
</template>
