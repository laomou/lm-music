  ...
  <section v-if="track" class="page now-playing">
    ...
    <div class="player-layout">
      <div class="player-main">
        ...
        <div class="progress-wrap"><input :value="player.currentTime" type="range" min="0" :max="player.duration || track.duration" step="0.1" :aria-label="t('player.seekProgress')" :aria-valuetext="`${formatDuration(player.currentTime)} / ${formatDuration(player.duration || track.duration)}`" @input="seek" /><div><time>{{ formatDuration(player.currentTime) }}</time><time>{{ (player.duration || track.duration) ? formatDuration(player.duration || track.duration) : '--:--' }}</time></div></div>
        <PlayerControls />
