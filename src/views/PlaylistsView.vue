...
<template>
  <section class="page playlists-page">
    ...
    <div class="section-heading"><h2>{{ auth.isConnected ? 'Jellyfin 歌单' : '已下载歌单' }}</h2><span v-if="library.loading">同步中…</span></div>
    ...
    <div v-else-if="!library.loading" class="empty-state"><p>{{ auth.isConnected ? '还没有可显示的音乐歌单。请在 Jellyfin 创建歌单后再试。' : '尚未连接 Jellyfin 服务器。' }}</p><button v-if="!auth.isConnected" class="primary-button" @click="router.push('/connect')">连接 Jellyfin</button></div>

    <div v-if="!auth.isConnected && library.playlists.length" class="connect-note"><span>当前显示的是已缓存的离线内容</span><button class="text-button" @click="router.push('/connect')">连接 Jellyfin</button></div>
  </section>
</template>
