...
<template>
  <section class="page playlists-page">
    ...
    <div v-if="player.recentTracks.length" class="recent-tracks">
      <button type="button" v-for="track in player.recentTracks.slice(0, 6)" :key="track.id" class="recent-track" @click="playRecent(track.id)">
    ...
    <div v-if="library.playlists.length" class="playlist-grid">
      <button type="button" v-for="playlist in library.playlists" :key="playlist.id" class="playlist-card" @click="router.push(`/playlist/${playlist.id}`)">
